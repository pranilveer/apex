      ...
      <CardContent className="p-4 sm:p-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onMonthChange(new Date(year, monthIndex - 1, 1))}>
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <p className="w-28 text-center text-sm font-medium sm:w-36">{MONTH_NAMES[monthIndex]} {year}</p>
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onMonthChange(new Date(year, monthIndex + 1, 1))}>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            const isToday = key(day) === todayStr
            return (
              <div
                key={day}
                className={cn(
